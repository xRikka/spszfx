...
public class ImgprocController {
    ...
    public List<Check> extractFeatures(BufferedImage buffImg, @PathVariable Long project_id, int minPeakDistance, double loUpDiff, double threshold){
    ...
    public List<Check> extractFeatures(String imgPath,@PathVariable Long project_id,int minPeakDistance,double loUpDiff,double threshold){
        Mat frame = IOUtil.readImg(imgPath);
        return imgprocService.extractFeatures(frame,project_id,minPeakDistance,loUpDiff,threshold);
    }

    @RequestMapping(value = "/extract/{project_id}/samples",method = RequestMethod.POST)
    public List<Check> extractFeaturesFromSamples(String samples_path,@PathVariable  Long project_id, int minPeakDistance, double loUpDiff, double threshold){
